 ...
 *
 * $Id: Event.php 41 2009-12-24 14:10:43Z bluelovers $
 *
 * @package    Core
 * @author     Kohana Team
...
abstract class Event_Core {
	...
	public static function run($name, & $data = NULL)
	{
		...
		if ( ! empty(Event::$events[$name]))
		{
			...
			(Event::$level < 2) && Event::$data =& $data;
			$callbacks  =  Event::get($name);

			foreach ($callbacks as $callback)
			{
//				is_callable( $callback );
				$retval = call_user_func_array($callback, array(&$data));
				
				if ($retval === false) {
					// stop this event
					break;
				} elseif (0 && is_string($retval)) {
					// error for this event
					
					echo Kohana::debug($name, $callback);
					
					Event::$has_run[$name] = $name;
//					throw new Kohana_Exception('[EVENT] Error: :name: - :retval:', array(':name:' => __($name), ':retval:' => __($retval)));
//					exit;
			...
			$clear_data = '';
			(Event::$level < 2) && Event::$data =& $clear_data;
